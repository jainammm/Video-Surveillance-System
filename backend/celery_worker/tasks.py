...
from service.live_stream.live_scene_detect import LiveStreamSceneDetect
...
def yolo_task(self, image_paths, db_id):
    total_objects = yolo_images(image_paths, os.path.join(
        os.path.dirname(image_paths), 'yolo'))
    insert_yolo_result(db_id, total_objects)


@app.task(track_started=True, bind=True)
def text_recognition_task(self, image_paths, db_id):
    allTextResults = east_text_recog_images(image_paths, os.path.join(
        os.path.dirname(image_paths), 'text_recognition'))
...
def face_recognition(self, video_path, face_path, db_id):
    ...
    for _, scene_image in image_filenames.items():
        face_recognition_results[scene_image[0]] = face_recog(
            face_path, scene_image[0])
        face_recognition_results[scene_image[1]] = face_recog(
            face_path, scene_image[1])
...
def live_stream_task(self, facePath, db_id, objectParameters, textParameters, email):
    ...
    liveStreamSceneDetect.find_scenes()
